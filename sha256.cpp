 ...
 * guarantee it works.
 * @version   $Id: sha256.cpp,v 1.6 2010/01/18 16:12:50 vtamara Exp $
 */
...
string hexaDeHash(unsigned const char *hash, int l)
{
        ...
        ss.clear();
        for (i=0; i< l; i++) {
                ss << setfill('0') << setw(2) << setbase(16) <<
                (uint32_t)hash[i];
        }
        return ss.str();
}
...
std::string sha256archivo(std::string narch)
{
        ...
        hash_state md;
        int len;
        unsigned char colchon[1024], hash[33];

        if ((file = fopen (narch.c_str(), "rb")) == NULL) {
                cerr << "sha256archivo no pudo abrir archivo '" <<
                narch << "'" << endl;
                exit(1);
        }
        ...
        while ( (len = fread (colchon, 1, 1024, file)) ) {
                sha256_process(&md, colchon, len);
        }

        sha256_done(&md, hash);
        fclose (file);

        return hexaDeHash(hash, 32);
}
...
#ifdef EJEMPLO_SHA256

int main()
{
        hash_state md;
        unsigned char *in = (unsigned char *)"hola mundo", out[33];
        sha256_init(&md);
        sha256_process(&md, (const unsigned char *)in, strlen((char *)in));
        sha256_done(&md, out);
        int i;
        for (i=0; i< 32; i++) {
                printf("%02x", out[i]);
